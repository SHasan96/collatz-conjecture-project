...
!> RECURSIVE function that finds the collatz sequence length of an integer
!@param num - an integer
recursive function find_sequence_length(num) result(steps)
  integer (kind = 8), intent(in) :: num
  ...
  if (m == 1) then
    ...
    steps = 1 + find_sequence_length(m / 2)
  else
    steps = 1 + find_sequence_length(3*m + 1)
  end if
